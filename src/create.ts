...
const insert = async <T>(
  ...
  if (typeof returning === 'string') returning = [returning]

  const result = (await model.db.arrays(
    `INSERT INTO ${model.quotedTable} (${columns.join(
      ', ',
    )}) VALUES ${values
      .map((row) => `(${row.join(', ')})`)
      .join(', ')} RETURNING ${returning.map((c) => `"${c}"`).join(', ')}`,
  )) as any[][]
  ...
  if (now) {
    if (modelColumns[createdAtColumnName] && modelColumns[updatedAtColumnName])
      records.forEach(
        (record: any) =>
          (record[createdAtColumnName] = record[updatedAtColumnName] = now),
      )
    else if (modelColumns[createdAtColumnName])
      records.forEach((record: any) => (record[createdAtColumnName] = now))
    else records.forEach((record: any) => (record[updatedAtColumnName] = now))
  }
...
const createMany = <T>(
  ...
  const values = records.map((record: any) =>
    columns.map((column) => quote(record[column])),
  )
...
export const create = async <T>(
  model: Model<any>,
  records: T | T[],
  returning: string | string[] = model.primaryKey,
) => {
  if (Array.isArray(records)) return await createMany(model, records, returning)
  else return (await createOne(model, records, returning))[0]
}
