...
export type IdParameter<Entity> =
  | number
  | string
  | Partial<Entity>
  | (number | string | Partial<Entity>)[]

const findByIdParameter = <T>(model: Model<T>, id: IdParameter<T>) => {
  if (typeof id === 'object')
    if (Array.isArray(id))
      id = id.map((record) =>
        typeof record === 'object' ? (record as any)[model.primaryKey] : record,
      )
    else id = (id as any)[model.primaryKey] as number | string
...
export interface Model<Entity> {
  ...
  create<T extends Partial<Entity>>(
    records: T,
  ...
  hasAndBelongsToMany<T extends Model<any>, P>(
    fn: (params: P) => T,
    options?: HasAndBelongsToManyOptions<T>,
  ): ScopeAll<T, P>

  relations<T extends { [key: string]: any }>(
    fn: (model: Model<Entity>) => T,
  ): this & T
  ...
  ): Omit<this, 'then'> & { then: Then<boolean> }
  _count(
    args?: string,
    options?: AggregateOptions,
  ): Omit<this, 'then'> & { then: Then<boolean> }
...
const porm = (db: Adapter, { camelCase = true } = {}) => <Entity>(
  table: string,
  klass: new (...args: any) => Entity = (class {} as new (...args: any) => Entity),
  options: Options = {},
) => {
  ...
  const self = {
    ...
    columns() {
      ...
      return ((this.model as any).columnsPromise = new Promise(
        async (resolve) => {
          const columns: Record<string, ColumnDefinition> = {}
          const array = (await db.query`SELECT * FROM information_schema.columns WHERE table_name = ${table}`) as ColumnDefinition[]
    ...
    update(id, set, returning) {
      return update(
        findByIdParameter<Entity>(this, id),
        set,
        returning,
        typeof id === 'object' ? (id as Entity) : undefined,
      )
    },
    ...
    relations(fn) {
      ...
      for (const key in relations) model[key] = relations[key](key)
      return model
    },
    ...
    scopes(scopes) {
      for (const key in scopes)
        (this as any)[key] = (...args: any[]) =>
          scopes[key].apply(this.toQuery(), args)
      return this
    },
    ...
    prepare(fn) {
      ...
      for (key in prepared) (this as any)[key] = prepared[key]()
      return this
    },

    all() {
      const q = this.toQuery()
      if (q.__query.take) return q.clone()._all()
      return q
    },

    _all() {
      const model = (this.toQuery() as any) as Omit<typeof self, 'then'> & {
        then: Then<Entity[]>
      }
      model.then = thenAll
    ...
    _rows() {
      const model = (this as any) as Omit<typeof self, 'then'> & {
        then: Then<any[][]>
      }
      model.then = thenRows
      return model
    ...
    clone() {
      return createQuery(
        (this as any).model || this,
        (this as any).__query as Query,
      )
    },
    ...
    _json() {
      ...
      let sql
      if (q.take) sql = `COALESCE(row_to_json("t".*), '{}') AS json`
      else sql = `COALESCE(json_agg(row_to_json("t".*)), '[]') AS json`
    ...
    _first(limit?: number) {
      if (limit) return this.order(this.primaryKey).limit(limit).all()
    ...
    _last(limit?: number) {
      if (limit)
        return this.order({ [this.primaryKey]: 'DESC' })
          .limit(limit)
          .all()
  ...
  self.belongsTo = (fn, options) => belongsTo(self, fn, options) as any

  self.hasOne = (fn, options) => hasOne(self, fn, options) as any

  self.hasMany = (fn, options) => hasMany(self, fn, options) as any
  ...
  const thenAll: Then<Entity[]> = function (resolve, reject) {
    return (this.db.query(this.toSql()) as Promise<Entity[]>).then(
      resolve,
      reject,
    )
  }
