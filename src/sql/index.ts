...
export const toSql = async ({
  ...
  if (query.distinct || query.distinctRaw) {
    sql.push('DISTINCT')
    if (distinctList.length) sql.push('ON', `(${distinctList.join(', ')})`)
  }

  if (!query.select && !query.selectRaw && model.hiddenColumns.length) {
    const hidden = model.hiddenColumns
    query.select = (await model.columnNames()).filter(
  ...
  else selectList.push(`${quotedAs}.*`)

  if (query.include) await include(selectList, model, as, query.include)
  ...
  if (query.join) query.join.forEach((args) => join(sql, model, as, args))
  ...
  if (query.having) sql.push('HAVING', having(query.having))

  if (query.window) sql.push('WINDOW', window(query.window))

  if (query.union) sql.push(await union('UNION', query.union))
  if (query.unionAll) sql.push(await union('UNION ALL', query.unionAll))
  if (query.intersect) sql.push(await union('INTERSECT', query.intersect))
  if (query.intersectAll)
    sql.push(await union('INTERSECT ALL', query.intersectAll))
  if (query.except) sql.push(await union('EXCEPT', query.except))
  if (query.exceptAll) sql.push(await union('EXCEPT ALL', query.exceptAll))
  ...
  if (query.for) sql.push('FOR', query.for)
