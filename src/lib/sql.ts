import {Model, QueryData} from "../types";
import {select} from './sql/select'
import {join} from './sql/join'
import {where} from './sql/where'
import {group} from './sql/group'
import {having} from './sql/having'
import {window} from './sql/window'
import {union} from './sql/union'
import {order} from './sql/order'

export const toSql = ({model, __query: query}: {model: Model, __query: QueryData}) => {
  const sql: string[] = []
