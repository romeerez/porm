...
describe('hasMany', () => {
    it('makes proper query', async () => {
        const chat = { id: 5 };
        expect(await Chat.messages(chat).toSql()).toBe(utils_1.line(`
      SELECT "messages".* FROM "messages"
      WHERE "messages"."chatId" = ${chat.id}
    `));
        expect(await Chat.messagesWithScope(chat).toSql()).toBe(utils_1.line(`
      SELECT "messages".* FROM "messages"
    ...
    it('can be joined', async () => {
        const q = Chat.all();
        expect(await q.join('messages').toSql()).toBe(utils_1.line(`
      SELECT "chats".* FROM "chats"
      JOIN "messages" ON "messages"."chatId" = "chats"."id"
    `));
        expect(await q.join('messagesWithScope').toSql()).toBe(utils_1.line(`
      SELECT "chats".* FROM "chats"
      JOIN "messages"
    ...
    it('has json subquery', async () => {
        expect(await Chat.messages.json().toSql()).toBe(utils_1.line(`
    ...
    `));
        expect(await Chat.messagesWithScope.json().toSql()).toBe(utils_1.line(`
      SELECT COALESCE(json_agg(row_to_json("t".*)), '[]') AS json
      FROM (
        SELECT "messages".* FROM "messages"
        WHERE "messages"."active" = true
